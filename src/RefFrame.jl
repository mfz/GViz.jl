...
Compute smallest `Rect` covering `r1` and `r2`
"""
...
translation by `(x, y)`
"""
# translate child by (x, y) from parent
...
struct Scaling <: AbstractTransform
    from::Rect # device (parent)
    to::Rect   # user   (child)
end


# compute parent coords from child coords
function user2dev(t::Translation, x, y)
    x .+ t.x, y .+ t.y
end

# compute child coords from parent coords
